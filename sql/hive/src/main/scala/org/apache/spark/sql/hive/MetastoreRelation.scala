...
private[hive] case class MetastoreRelation(
  ...
  override def computeStats(
    ...
    val rowEstimate: Option[BigInt] = if (catalogTable.partitionColumnNames.nonEmpty) {
      ...
      // Get row count generated by Hive or ANALYZE TABLE command
      catalogTable.properties.get("numRows").map(_.toLong)
        .orElse(catalogTable.stats.flatMap(_.rowCount).map(_.toLong)) match {
