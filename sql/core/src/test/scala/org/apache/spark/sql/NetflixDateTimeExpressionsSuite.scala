...
class NetflixDateTimeExpressionsSuite extends QueryTest with SharedSQLContext {
  ...
  test("nf_hour") {
    withTimeZone("UTC") {
      checkAnswer(sql("SELECT nf_hour(20180531)"), Row(0) :: Nil)
      checkAnswer(sql("SELECT nf_hour('20180531')"), Row(0) :: Nil)
      checkAnswer(sql("SELECT nf_hour('2018-05-31')"), Row(0) :: Nil)
      checkAnswer(sql("SELECT nf_hour(date '2018-05-31')"), Row(0) :: Nil)
      checkAnswer(sql("SELECT nf_hour(timestamp '2018-01-01 21:00:00.000')"), Row(21) :: Nil)
      checkAnswer(sql("SELECT nf_hour('2018-05-31T12:20:21.010')"), Row(12) :: Nil)
      checkAnswer(sql("SELECT nf_hour('2018-05-31 12:20:21.010')"), Row(12) :: Nil)
      checkAnswer(sql("SELECT nf_hour(null)"), Row(null) :: Nil)
    }
  }
