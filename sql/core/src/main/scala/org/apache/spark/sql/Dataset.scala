...
import org.apache.spark.sql.execution.{FileRelation, LogicalRDD, QueryExecution, SparkPlan, SQLExecution}
...
class Dataset[T] private[sql](
    ...
    queryExecution.analyzed match {
      ...
      case p if hasSideEffects(p) =>
        LogicalRDD(queryExecution.analyzed.output,
          withAction("command", queryExecution)(_.execute()))(sparkSession)
  ...
  def materialize(): Dataset[T] = withCallback("materialize", toDF()) { _ =>
    withNewExecutionId {
