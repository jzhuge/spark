...
object DataSourceV2Relation {
  ...
  def create(
    ...
    val output = schema(source, makeV2Options(options, path, table), userSpecifiedSchema)
      .toAttributes
    DataSourceV2Relation(source, output, options, path, table, userSpecifiedSchema)
  }
