  ...
  def run(
    ...
    Utils.tryWithSafeFinallyAndFailureCallbacks(block = {
      ...
      val msg = if (useCommitCoordinator) {
        ...
        val commitAuthorized = coordinator.canCommit(stageId, stageAttempt, partId, tid)
        if (commitAuthorized) {
          logInfo(s"Writer for stage $stageId.$stageAttempt, " +
              s"task $partId (TID $tid) is authorized to commit.")
          dataWriter.commit()

        } else {
          val message = s"Stage $stageId.$stageAttempt, " +
              s"task $partId (TID $tid): driver did not authorize commit"
          logInfo(message)
