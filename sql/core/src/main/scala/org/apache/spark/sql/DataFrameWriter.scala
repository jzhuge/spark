...
import org.apache.spark.sql.catalyst.plans.logical.{CreateTableAsSelect, ReplaceTableAsSelect}
  ...
  def save(): Unit = {
    ...
    extraOptions.get("catalog") match {
      case Some(catalogName) if extraOptions.get(DataSourceOptions.TABLE_KEY).isDefined =>
        val catalog = df.sparkSession.catalog(Some(catalogName)).asTableCatalog
