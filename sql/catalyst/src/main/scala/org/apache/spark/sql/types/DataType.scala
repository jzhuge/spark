...
object DataType {
   ...
   *
   * Backported from ffc57b0 for datasource v2 API.
   */
  ...
  // 2.1.1 doesn't have Cast.canSafeCast, so this logic is stolen from ResolveUpCast
  def canSafeCast(f: AtomicType, t: AtomicType): Boolean = {
    if (Cast.canCast(f, t)) {
      (f, t) match {
