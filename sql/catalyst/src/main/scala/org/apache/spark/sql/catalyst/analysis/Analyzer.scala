  ...
  object ResolveRelations extends Rule[LogicalPlan] {
      ...
      case i @ InsertIntoTable(u: UnresolvedRelation, parts, child, overwrite, ifNotExists, _)
        ...
        table match {
          case v2: V2Relation =>
            ...
            assert(!overwrite.enabled, s"Cannot overwrite with table: $v2")
    ...
    private def updateWithTablePartitions(
      ...
      if (insert.isFromDataFrame) {
        ...
        val (inputPartCols, columns) = insert.child.output.partition { attr =>
        ...
        insert.copy(
          ...
          child = Project(columns ++ dynamicPartColumns, insert.child))
      } else {
