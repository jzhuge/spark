...
import org.apache.spark.sql.catalyst.{FunctionIdentifier, InternalRow, MultipartTableIdentifier, TableIdentifier, TableIdentifierLike}
...
object UnresolvedRelation {
  def apply(parts: Seq[String]): UnresolvedRelation =
    UnresolvedRelation(MultipartTableIdentifier(parts))
}
