...
import org.apache.spark.sql.catalyst.{FunctionIdentifier, InternalRow, MultipartIdentifier, TableIdentifier, TableIdentifierLike}
...
object UnresolvedRelation {
  def apply(parts: Seq[String]): UnresolvedRelation =
    UnresolvedRelation(MultipartIdentifier(parts))
}
