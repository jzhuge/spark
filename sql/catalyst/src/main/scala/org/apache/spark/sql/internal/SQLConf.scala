...
object SQLConf {
  ...
  val ADVANCED_PARTITION_PREDICATE_PUSHDOWN =
    SQLConfigBuilder("spark.sql.hive.advancedPartitionPredicatePushdown.enabled")
      .internal()
  ...
  val PARQUET_OUTPUT_TIMESTAMP_TYPE = SQLConfigBuilder("spark.sql.parquet.outputTimestampType")
  ...
  val PARQUET_INT64_AS_TIMESTAMP_MILLIS =
    SQLConfigBuilder("spark.sql.parquet.int64AsTimestampMillis")
  ...
  val PARQUET_COMPRESSION = SQLConfigBuilder("spark.sql.parquet.compression.codec")
    ...
    .checkValues(Set("uncompressed", "snappy", "gzip", "lzo", "brotli"))
  ...
  val S3_COMMIT_PROTOCOL_CLASS =
    SQLConfigBuilder("spark.sql.sources.s3.commitProtocolClass")
        .internal()
        .stringConf
        .createWithDefault(
          "org.apache.spark.sql.execution.datasources.S3MapReduceCommitProtocol")

  val USE_S3_OUTPUT_COMMITTER = SQLConfigBuilder("spark.sql.s3committer.enabled")
      .doc("When true, use the S3 output committer for S3 paths.")
      .booleanConf
      .createWithDefault(true)

  val USE_S3_BATCH_OUTPUT_COMMITTER = SQLConfigBuilder("spark.sql.batch.s3committer.enabled")
      .doc("When true, use the S3 batch output committer for S3 tables.")
      .booleanConf
      .createWithDefault(true)

  val METACAT_CATALOG = SQLConfigBuilder("spark.sql.metacat.write.catalog")
