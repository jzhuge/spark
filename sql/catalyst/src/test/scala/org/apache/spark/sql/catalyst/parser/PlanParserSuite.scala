...
class PlanParserSuite extends AnalysisTest {
  ...
  test("insert into") {
    ...
    def insert(
      ...
      InsertIntoTable(table("s"), partition, plan, overwrite, ifPartitionNotExists, Map.empty)
    ...
    assertEqual("from t insert into s select * limit 1 insert into u select * where x > 5",
      InsertIntoTable(
        table("s"), Map.empty, plan.limit(1), false, ifPartitionNotExists = false, Map.empty).union(
        InsertIntoTable(
          table("u"), Map.empty, plan2, false, ifPartitionNotExists = false, Map.empty)))
  }
      ...
      InsertIntoTable(table("s"), Map.empty,
        UnresolvedHint("REPARTITION", Seq(Literal(100)),
          UnresolvedHint("COALESCE", Seq(Literal(500)),
            UnresolvedHint("COALESCE", Seq(Literal(10)),
              table("t").select(star())))),
        overwrite = false, ifPartitionNotExists = false, options = Map.empty))
