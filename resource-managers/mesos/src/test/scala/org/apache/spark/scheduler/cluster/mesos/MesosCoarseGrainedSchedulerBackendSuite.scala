...
class MesosCoarseGrainedSchedulerBackendSuite extends SparkFunSuite
  ...
  test("mesos declines offers with a filter when reached spark.cores.max with executor.cores") {
    ...
    verifyTaskLaunched(driver, "o1")
    verifyTaskLaunched(driver, "o2")
    verifyDeclinedOffer(driver, createOfferId("o3"), createFilter(refusalLimit))
  }
  ...
  test("supports spark.scheduler.minRegisteredResourcesRatio when executor cores is specified") {
    // max cores is not a multiple of executor cores, so we assert that the isReady check
    // is against the adjusted true maxCores.
    val maxCores = 4
    val executorCores = 3
    setBackend(Map(
      "spark.cores.max" -> maxCores.toString,
    ...
    val offers = List(Resources(backend.executorMemory(sc), executorCores))
    offerResources(offers)
    val launchedTasks = verifyTaskLaunched(driver, "o1")
    assert(!backend.isReady)

    registerMockExecutor(launchedTasks(0).getTaskId.getValue, "s1", executorCores)
    assert(backend.isReady)
  }
